 ...
 * *	April.29.2015-Combined UDP header check function with DNS function
 * */
/**********************************************************************/
...
// Process DNS Query
//int process_udp_dns(struct ioq_header *ioq, struct ether_header *eth, struct iphdr *ip, struct pkt_buff *pkt)
int process_udp_dns(struct ioq_header *ioq, struct ether_header *eth, struct iphdr *ip, struct pkt_buff *pkt, Trie *root)
{
...
//acc=0;
//// udp
//memcpy((void *) rudp, (void *) udp, htons(ioq->byte_length) - sizeof(struct ether_header) - sizeof(struct iphdr)); // Push the internal buffer msg to pkt
//rudp->source = udp->dest;
//rudp->dest = udp->source;
//rudp->len    = htons(ntohs(rip->tot_len) - sizeof(struct iphdr));
//// init checksum to zero to calcualate
...
//	}
	...
	switch(ntohs(ip->protocol))
	{
		...
		case IPPROTO_UDP:
			...
			result = process_udp_dns(ioq, eth, ip, pkt, root);
			break;
		default:
			result = 1;
			break;
...
int main(void)
{
	...
	memcpy(rarp->arp_spa, &iface.ip, 4);
		// target
	memcpy(rarp->arp_tha, dest_mac, ETH_ALEN);
	memcpy(rarp->arp_tpa, dest_ip2, 4);
	...
	memcpy(rarp->arp_spa, &iface.ip, 4);
		// target
	memcpy(rarp->arp_tha, dest_mac, ETH_ALEN);
	memcpy(rarp->arp_tpa, dest_ip3, 4);
