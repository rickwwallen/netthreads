...
Trie *readZone(char *fn)
{
	...
	//RR *rrs;
	printf("Trie *load_zone()\n{\n");
	printf("\tchar domNme[DNM_SZ];\n");
	printf("\tRR *rrs;\n");
	printf("\tuint32_t dTtl = 0; //default ttl gets redefined by SOA\n");
	printf("\tuint16_t dClass = 0; //default class gets redefined by SOA\n");
	printf("\tchar rR2[LNE_SZ];\n");
	printf("\tTrie *root;\n\n");

	printf("\troot = createNode('*', NULL);\n\n");
	...
	while(!feof(fp))
	{
		...
		if(strcmp(rR2, "") != 0)
		{
			...
			{
				printf("\tmemset(&domNme, '\\0', DNM_SZ);\n");
				printf("\tmemset(&rR2, '\\0', LNE_SZ);\n");
				printf("\tmemcpy(&domNme, \"%s\", %d);\n", domNme,(int) strlen(domNme));
				printf("\tmemcpy(&rR2, \"%s\", %d);\n", rR2, (int) strlen(rR2));
				//printf("\tprintf(\"s\\n\", domNme);\n");
				//printf("\tprintf(\"s\\n\", rR2);\n");
				//printf("\tdTtl = %d;\n", dTtl);
				//printf("\tdClass = %d;\n",dClass);
				//printf("\taddTrie(root, domNme, rR2, &dTtl, &dClass);\n\n");
				printf("\trrs = createResRec(rR2, &dTtl, &dClass);\n");
				//printf("\tprintf(\"rrs\\n\");\n");
				printf("\tif(rrs != NULL)\n\t{\n");
				printf("\t\taddTrie(root, domNme, rrs);\n");
				printf("\t}\n");
				printf("\trrs = NULL;\n\n");
			}
